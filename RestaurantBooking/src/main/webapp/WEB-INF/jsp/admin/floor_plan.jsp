...
<c:set var="embedded" value="${param.embedded eq '1'}" />
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
	...
	<div class="floorplan-main">
		...
		<div class="floorplan-header">
			...
			<div class="floorplan-controls">
				<label for="dateSelect">View Date:</label> <input type="date"
					id="dateSelect" value="${selectedDate}"
					onchange="changeDate(this.value)"> <a
					href="<c:url value='/admin/add_table'/>"
					class="btn btn-primary btn-small">+ Add Table</a>
			</div>
		</div>
		...
		<c:if test="${empty tables}">
			<p>
				No tables found. <a href="<c:url value='/admin/add_table'/>">Add
					some tables</a> to get started.
			</p>
		</c:if>
	...
	<div id="tableModal" class="modal">
		<div class="modal-content">
			...
			<div class="modal-actions">
				<a id="editTableLink" href="#" class="btn btn-secondary btn-small">Edit
					Table</a>
				<button id="deleteTableBtn" onclick="deleteTable()"
					class="btn btn-danger btn-small">Delete Table</button>
			</div>
		</div>
	</div>
...
// true if we're inside the iframe (called with ?embedded=1)
var isEmbedded = ${embedded ? 'true' : 'false'};

function changeDate(date) {
    var base = '${pageContext.request.contextPath}/admin/floor_plan';
    if (isEmbedded) {
        window.location.href = base + '?embedded=1&date=' + date;
    } else {
        window.location.href = base + '?date=' + date;
    }
}
...
function showTableDetails(tableId, tableNumber, status) {
    ...
    .then(data => {
        if (data.bookings && data.bookings.length > 0) {
            ...
            data.bookings.forEach(booking => {
                ...
                if (!customerName && booking.customerEmail) {
    ...
    .catch(error => {
        modalBody.innerHTML =
          '<p class="flash-error">Error loading bookings: ' + error.message + '</p>';
    });
