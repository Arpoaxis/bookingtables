...
public class EmployeeManageServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// Allow both ADMIN and MANAGER to view/manage employees
		...
		try {
			...
			if (roleFilter != null && !roleFilter.isBlank() && !"ALL".equalsIgnoreCase(roleFilter)) {
				...
				List<User> filtered = new java.util.ArrayList<>();
				for (User u : employees) {
					if (u.getAccountType() != null && u.getAccountType().equalsIgnoreCase(wanted)) {
						filtered.add(u);
					}
				}
				employees = filtered;
			...
			if ("name".equalsIgnoreCase(sort)) {
				cmp = java.util.Comparator.comparing((User u) -> u.getLastName() == null ? "" : u.getLastName())
						.thenComparing(u -> u.getFirstName() == null ? "" : u.getFirstName());
			} else if ("email".equalsIgnoreCase(sort)) {
				cmp = java.util.Comparator.comparing((User u) -> u.getEmail() == null ? "" : u.getEmail(),
						String.CASE_INSENSITIVE_ORDER);
			} else if ("role".equalsIgnoreCase(sort)) {
				cmp = java.util.Comparator.comparing((User u) -> u.getAccountType() == null ? "" : u.getAccountType(),
						String.CASE_INSENSITIVE_ORDER);
		...
		try {
			...
			if ("create".equals(action)) {
				...
				if (firstName == null || firstName.isBlank() || lastName == null || lastName.isBlank() || email == null
						|| email.isBlank() || phoneStr == null || phoneStr.isBlank()) {
				...
				if (!"EMPLOYEE".equals(normalizedRole) && !"HOST".equals(normalizedRole)
			...
			if ("resetPassword".equals(action)) {
				...
				session.setAttribute("success",
						"Password reset to 'changeme'. Ask the employee to change it on next login.");
